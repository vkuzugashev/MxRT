...
#ifdef _DEBUG
#ifdef _WIN64
#pragma comment(lib,"../x64/Debug/OPCClientToolKit64D.lib")
#else
#pragma comment(lib,"../Debug/OPCClientToolKit32D.lib")
#endif
#endif

#ifdef NDEBUG
#ifdef _WIN64
#pragma comment(lib,"../x64/Release/OPCClientToolKit64.lib")
#else
#pragma comment(lib,"../Release/OPCClientToolKit32.lib")
#endif
#endif
...
bool LocalSyncOPCCLient::SyncReadItem(std::string item_name, VARIANT* var, uint16_t& wqlt, FILETIME& timeVal)
{
	static OPCItemData data;
	//name_item_map_[item_name]->readSync(data, OPC_DS_DEVICE);
	name_item_map_[item_name]->readSync(data, OPC_DS_CACHE);
	VariantCopy(var, &data.vDataValue);
	wqlt = data.wQuality;
	timeVal = data.ftTimeStamp;
	return true;
}

bool LocalSyncOPCCLient::ReadBool(std::string item_name, uint16_t& wqlt, FILETIME& timeVal)
{
	SyncReadItem(item_name, &read_tmp_variant_, wqlt, timeVal);
...
float LocalSyncOPCCLient::ReadFloat(std::string item_name, uint16_t& wqlt, FILETIME& timeVal)
{
	SyncReadItem(item_name, &read_tmp_variant_, wqlt, timeVal);
...
int16_t LocalSyncOPCCLient::ReadInt16(std::string item_name, uint16_t& wqlt, FILETIME& timeVal)
{
	SyncReadItem(item_name, &read_tmp_variant_, wqlt, timeVal);
	return read_tmp_variant_.iVal;
}

int32_t LocalSyncOPCCLient::ReadInt32(std::string item_name, uint16_t& wqlt, FILETIME& timeVal)
{
	SyncReadItem(item_name, &read_tmp_variant_, wqlt, timeVal);
	return read_tmp_variant_.intVal;
}


uint16_t LocalSyncOPCCLient::ReadUint16(std::string item_name, uint16_t& wqlt, FILETIME& timeVal)
{
	SyncReadItem(item_name, &read_tmp_variant_,wqlt, timeVal);
...
bool LocalSyncOPCCLient::ReadUint16Array(std::string item_name, uint16_t* item_value_array, int array_size)
{
	...
	SyncReadItem(item_name, &array_variant, wqlt, timeVal);
