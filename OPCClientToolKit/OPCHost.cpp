	...
	if (SUCCEEDED(result))
	{
		...
		if (SUCCEEDED(result))
		{
			...
			else
			{
				...
				{
					sprintf_s(buff, "Failed sz:(%s) classIdString(%s)\n", sz, classIdString);
					MxLog::LogWarn(buff);
				}
			}
		}
...
void CRemoteHost::getListOfDAServers(CATID cid, std::vector<std::string>& listOfProgIDs)
{
	...
	while ((result = iEnum->Next(1, &glist, &actual)) == S_OK)
	{
		...
		else
		{
			...
			COLE2T str(progID);
			char buff[500];
			sprintf_s(buff, "ProgID: %s ", str.m_psz);
			MxLog::LogInfo(buff);

			sprintf_s(buff, "CLSID: {%08lX-%04hX-%04hX-%02hhX%02hhX-%02hhX%02hhX%02hhX%02hhX%02hhX%02hhX}\n",
